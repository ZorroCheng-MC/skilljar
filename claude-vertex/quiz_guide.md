# Claude with Google Cloud's Vertex AI — Quiz Guide
...
**1. You want Claude to analyze a product image for 6 different materials at once. Instead of one huge prompt, what should you do?**

- A) Write a longer, more detailed prompt
- B) Send 6 separate requests in parallel, then combine results
- C) Ask the user to pick one material first
- D) Use an agent with material tools


**2. You're building a system where Claude creates content, then checks if it's good enough, then improves it if needed. What pattern is this?**
...
**3. You're building an app where users upload photos and always get the same 4-step process to enhance them. Which approach should you use?**

- A) An agent with photo tools
- B) A single complex prompt
- C) Multiple agents working together
- D) A workflow with predefined steps


**4. Your app needs to handle both "cooking recipes" and "workout routines" with completely different styles. What pattern helps?**

- A) Combine both styles in one prompt
- B) Use the same prompt for both
- C) Routing - categorize first, then use specialized prompts
- D) Always ask users to specify the category


**5. What defines an agent when working with Claude?**

- A) A predetermined sequence of steps that Claude must follow exactly
- B) A setup where Claude is given a goal and tools, then figures out how to complete the goal
- C) A system that categorizes user requests into different types
- D) A method for breaking complex tasks into parallel subtasks


**6. What is environment inspection in the context of AI agents?**

- A) The process of categorizing user requests into different workflow types
- B) The technique of running multiple specialized tasks simultaneously
- C) A method for breaking large tasks into smaller sequential steps
- D) Claude's ability to observe and understand the results of its actions


**7. You're building an agent. In general, should you give it a "Refactor Code" tool or basic tools like "read file" and "write file"?**

- A) "Refactor Code" tool - it's more specific
- B) Basic tools - they're more flexible
- C) Both tools together
- D) Neither - agents don't need tools
